...
// we actually do not use an interrupt here so we use a non used pin (PORTC.15)
...
static const struct pios_mpu6000_cfg pios_mpu6000_cfg = {
    ...
    .orientation    = PIOS_MPU6000_TOP_90DEG,
...
void PIOS_Board_Init(void)
{
    ...
    // make sure the motors do not spin:
    for (unsigned int i = 0; i < 4; i++) {
        GPIO_InitTypeDef GPIO_InitStructure = pios_tim_servoport_all_pins[i].pin.init;
        // overwrite to active output:
        GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
        GPIO_Init(pios_tim_servoport_all_pins[i].pin.gpio, &GPIO_InitStructure);
        // write to low:
        GPIO_WriteBit(pios_tim_servoport_all_pins[i].pin.gpio, pios_tim_servoport_all_pins[i].pin.init.GPIO_Pin, Bit_RESET);
    }
...
// #endif /* PIOS_INCLUDE_USB */
    ...
    // as we do not have usb, we always set up the com bridge here!
    uint32_t pios_usart_generic_id;
#if defined PIOS_INCLUDE_DEBUG_CONSOLE
    // if we enable debug we remap the serial connection to an unused port
    // as we do not want to mix gcs data ande debugging
    if (PIOS_USART_Init(&pios_usart_generic_id, &pios_usart_generic_not_connected_cfg)) {
        PIOS_Assert(0);
    }
#else
    ...
    case HWSETTINGS_CC_MAINPORT_TELEMETRY:
        ...
        {
            // we set up the telemetry link above as we do not have a usb connection
            NOT FIXED
            uint32_t pios_usart_generic_id;
            if (PIOS_USART_Init(&pios_usart_generic_id, &pios_usart_generic_main_cfg)) {
                PIOS_Assert(0);
    ...
    {
        // as we do not have usb, we always set up the com bridge above!
        /*
           uint32_t pios_usart_generic_id;
           if (PIOS_USART_Init(&pios_usart_generic_id, &pios_usart_generic_main_cfg)) {
            PIOS_Assert(0);
...
#if defined(PIOS_INCLUDE_TELEMETRY_RF)
        {
            /* as we do not have the usb option, we always set this up above!
                uint32_t pios_usart_generic_id;
    ...
    // NOT USED ON CJMCU GPIO_PinRemapConfig(GPIO_Remap_SWJ_NoJTRST, ENABLE);
...
#if defined(PIOS_INCLUDE_DEBUG_CONSOLE)
    // enable debugging on our main serial port:
    uint32_t pios_usart_generic_id2;

    // NOTE: when debugging is enabled there is no active gcs connection!
    if (PIOS_USART_Init(&pios_usart_generic_id2, &pios_usart_generic_main_cfg)) {
        PIOS_Assert(0);
    }
    uint8_t *tx_buffer2 = (uint8_t *)pios_malloc(PIOS_COM_DEBUGCONSOLE_TX_BUF_LEN);
    PIOS_Assert(tx_buffer);
    if (PIOS_COM_Init(&pios_com_debug_id, &pios_usart_com_driver, pios_usart_generic_id2,
                      NULL, 0,
                      tx_buffer2, PIOS_COM_DEBUGCONSOLE_TX_BUF_LEN)) {
        PIOS_Assert(0);
    }

    // PIOS_COM_SendFormattedString(pios_com_telem_rf_id, "DEBUG IF READY\r\n");
    DEBUG_PRINTF(0, "DEBUG OK\r\n");
#endif
    ...
    case BOARD_REVISION_CJMCU:
#if defined(PIOS_INCLUDE_MPU6000)
#if 1
        // no spi used, use i2c instead:
        if (PIOS_I2C_Init(&pios_i2c_mpu6000_adapter_id, &pios_i2c_mpu6000_adapter_cfg)) {
            PIOS_DEBUG_Assert(0);
        }
        ...
        PIOS_MPU6000_Init(pios_i2c_mpu6000_adapter_id, PIOS_MPU6000_I2C_ADDR, &pios_mpu6000_cfg);
        PIOS_MPU6000_CONFIG_Configure();
        init_test = !PIOS_MPU6000_Driver.test(0);
#else
        ...
        GPIO_Init(pios_i2c_mpu6000_adapter_cfg.scl.gpio, &pios_i2c_mpu6000_adapter_cfg.scl.init);
        GPIO_Init(pios_i2c_mpu6000_adapter_cfg.sda.gpio, &pios_i2c_mpu6000_adapter_cfg.sda.init);
        ...
        // GPIO_PinRemapConfig(pios_i2c_mpu6000_adapter_cfg.remap, ENABLE);

        I2C_Init(I2C1, &pios_i2c_mpu6000_adapter_cfg.init);
        ...
        I2C_Send7bitAddress(I2C1, PIOS_MPU6000_I2C_ADDR << 1, I2C_Direction_Transmitter);
